 ...
 * @param {String} input JavaScript source code
 */
Loader.transformLess = function (input, callback) {
 ...
 * @param {String} input JavaScript source code
 */
Loader.transformStylus = function (input, callback) {
 ...
 */
Loader.minify = function (basedir, arr, minifyCallback) {
  var resArr = [], alen = arr.length, i= 0;
  arr.forEach(function (item, index) {
    // combo
    var content = "";
    var minified = "";

    var assetsCount = item.assets.length;

    var afterAssetsCompile = function(j){
      if(j < assetsCount -1)
        return;
      ...
      if(++i == alen){
        minifyCallback(resArr);
      }
    };

    item.assets.forEach(function (asset, j) {
      var cached = cache[asset];
      // 编译，压缩
      if (!cached) {
        var file = path.join(basedir, asset);
        var text = fs.readFileSync(file, 'utf-8'), result;
        ...
        var cb = function(text){
          ...
          afterAssetsCompile(j);
        };

        if (extname === '.less') {
          Loader.transformLess(text, cb);
        } else if (extname === '.styl') {
          Loader.transformStylus(text, cb);
        } else if (extname === '.coffee') {
          cb(Loader.transformCoffee(text));
        } else{
          cb(text);
        }
